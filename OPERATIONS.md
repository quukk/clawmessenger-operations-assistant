...
# 场景2：融云消息收不到
# 查看 worker 日志（融云连接状态）
grep -i "rongcloud\|融云\|connect" $INSTALL_DIR/logs/worker-$(date +%Y-%m-%d).log | tail -50

# 场景3：设备控制命令无响应
# 查看 worker 日志（P2P 消息处理）
grep -i "device_control\|command\|P2P" $INSTALL_DIR/logs/worker-$(date +%Y-%m-%d).log | tail -50

# 场景4：服务频繁重启
# 查看 daemon 日志（崩溃原因）
grep -i "crash\|exit\|error" $INSTALL_DIR/logs/daemon-$(date +%Y-%m-%d).log | tail -50

# 场景5：自动更新失败
# 查看 updater 日志
tail -n 100 $INSTALL_DIR/logs/updater-$(date +%Y-%m-%d).log
```
