...
class Updater {
  ...
  async check() {
    ...
    // 如果更新 URL 是占位符，跳过检查
    if (CONFIG.UPDATE_URL.includes('your-cdn.com')) {
      log.info('[UPDATER] 更新 URL 未配置，跳过版本检查');
      return null;
    }
      ...
      const req = client.get(url, { timeout: 15000 }, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            log.warn(`[UPDATER] 版本接口返回非法JSON: ${data.substring(0, 100)}`);
            resolve(null); // 不抛出错误，只是跳过本次检查
          }
        });
      });

      req.on('error', (err) => {
        log.warn(`[UPDATER] 版本检测请求失败: ${err.message}`);
        resolve(null);
      });
      req.on('timeout', () => {
        req.destroy();
        log.warn('[UPDATER] 版本检测超时');
        resolve(null);
      });
    });
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      exec(psCmd, { timeout: 60000 }, (err, stdout, stderr) => {
        if (err) {
          ...
          exec(sevenz, { timeout: 60000 }, (err2) => {
